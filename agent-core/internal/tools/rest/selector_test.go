...
func TestResponseSelectorsShareNestedMapSemantics(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "array index", selector: "$.items.0.id", want: "first"},
		{name: "array index out of range", selector: "$.items.1.id"},
		{name: "key on an array", selector: "$.items.id"},
		{name: "signed index is a key", selector: "$.items.-0.id"},
		{name: "digits on an object stay a key", selector: "$.data.0"},
